...
async def search_articles(topic: str, max_results: int = 25) -> List[Dict]:
    ...
    if not NEWS_API_KEY:
        raise ValueError("NEWSAPI_ORG_KEY not found in environment variables")
...
async def search_guardian_articles(topic: str, max_results: int = 25) -> List[Dict]:
    """Search for historical articles from The Guardian"""
    
    if not GUARDIAN_API_KEY:
        print("Warning: GUARDIAN_API_KEY not found, skipping historical articles")
        return []
